...
public class GithubPluginUpdater(Lazy<Version> version) {
	public const string RepositoryName = "maxisoft/ASFUnpaidStuff";

	// Disabled by default: after the rename there is no GitHub repository publishing ASFUnpaidStuff releases yet.
	// Flip to true once RepositoryName points at a repo with release zips, otherwise the updater would silently fail (or pull a foreign build).
