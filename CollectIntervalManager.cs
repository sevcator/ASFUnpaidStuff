...
/// <inheritdoc />
/// <summary>
/// An interface that provides methods to manage the collect interval for the ASFUnpaidStuffPlugin.
/// </summary>
...
internal sealed class CollectIntervalManager(IASFUnpaidStuffPlugin plugin) : ICollectIntervalManager {
	...
	// The timer instance, guarded by TimerLock: RegisterBot/RemoveBot run concurrently from per-bot callback loops
	private Timer? Timer;
	private readonly object TimerLock = new();

	// Set by StopTimer so an in-flight tick cannot resurrect the timer afterwards
	...
	public TimeSpan RandomlyChangeCollectInterval(object? source) {
		...
		lock (TimerLock) {
			if (Timer is not null) {
				// Reschedule the existing timer in place to avoid per-tick Timer churn and the transient null window
				Timer.Change(delay, delay);
			}
			else if (!Stopped) {
				// Timer vanished without an explicit stop — recreate it; a stopped manager must stay stopped
				ResetTimer(() => new Timer(plugin.CollectGamesOnClock, source, delay, delay));
			}
		}
