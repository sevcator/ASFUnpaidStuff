...
public class ListFreeGamesMainStrategy : IListFreeGamesStrategy {
	private readonly OldRedditListFreeGamesStrategy OldRedditStrategy = new();
	...
	private async Task<IReadOnlyCollection<RedditGameEntry>> DoGetGames([NotNull] ListFreeGamesContext context, CancellationToken cancellationToken) {
		...
		try {
			...
			try {
				IReadOnlyCollection<RedditGameEntry> oldRedditResult = await OldRedditStrategy.GetGames(
					context with {
						HttpClient = new Lazy<SimpleHttpClient>(() => context.HttpClientFactory.CreateForReddit())
					}, cts.Token
				).ConfigureAwait(false);

				if (oldRedditResult.Count > 0) {
					context.PreviousSucessfulStrategy = EListFreeGamesStrategy.OldReddit;
					await CancelAndObserveTasks(cts, redditTask1).ConfigureAwait(false);

					return oldRedditResult;
				}
			}
			catch (Exception) {
				// ignored and handled below by the existing reddit/redlib fallbacks
			}
...
#pragma warning restore CA1849
	...
	private async Task<IReadOnlyCollection<RedditGameEntry>> LastTryRedditStrategy(ListFreeGamesContext context, Task firstTryTask, CancellationToken cancellationToken) {
		...
		return await RedditStrategy.GetGames(
			context with {
				Retry = context.Retry > 0 ? context.Retry - 1 : 0,
				HttpClient = new Lazy<SimpleHttpClient>(() => context.HttpClientFactory.CreateForReddit())
			}, cancellationToken
		).ConfigureAwait(false);
	}
	...
	private static async Task CancelAndObserveTasks(CancellationTokenSource cancellationTokenSource, params Task[] tasks) {
		...
		foreach (Task task in tasks) {
			try {
				...
				// ignored; observing background source failures prevents UnobservedTaskException noise
			}
		}
	}
}
