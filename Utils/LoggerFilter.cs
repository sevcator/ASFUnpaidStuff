...
public partial class LoggerFilter {
    private static readonly object GlobalFilterLock = new();
    private static readonly WhenMethodFilter SensitiveConfigDumpFilter = new(IgnoreSensitiveConfigDump);
    private static bool SensitiveConfigDumpFilterInstalled;
    ...
    public static void InstallSensitiveConfigDumpFilter() {
        lock (GlobalFilterLock) {
            if (SensitiveConfigDumpFilterInstalled) {
                return;
            }
            ...
            foreach (LoggingRule loggingRule in config.LoggingRules) {
                loggingRule.Filters.Insert(0, SensitiveConfigDumpFilter);
            }

            SensitiveConfigDumpFilterInstalled = true;
    ...
    [GeneratedRegex(@"^(?:config\\)?[^:\r\n]+\.(?:json|db|config):\s*[\{\[]", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant)]
    private static partial Regex SensitiveConfigDumpRegex();
    ...
    private FilterResult FilterLogEvent(LogEventInfo eventInfo) {
        // The NLog logger name is already the bot name (or "ASF"); look it up directly instead of resolving the Bot per event.
        if (Filters.TryGetValue(eventInfo.LoggerName ?? "", out LinkedList<Func<LogEventInfo, bool>>? filters)) {
    ...
    private static FilterResult IgnoreSensitiveConfigDump(LogEventInfo eventInfo) {
        if (eventInfo.Level != LogLevel.Debug) {
            return FilterResult.Log;
        }

        string message = eventInfo.FormattedMessage ?? eventInfo.Message;

        return SensitiveConfigDumpRegex().IsMatch(message) ? FilterResult.IgnoreFinal : FilterResult.Log;
    }
